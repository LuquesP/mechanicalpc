...
from util.utils import repsurf_get_model, repsurf_get_loss, set_seed, weight_init
...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        default="repsurf.scanobjectnn.repsurf_ssg_umb",
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="batch size in training [default: 64]",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
    ...
    parser.add_argument(
        "--scheduler", type=str, default="step", help="scheduler for training"
    )
    parser.add_argument(
        "--epoch",
        default=500,
    ...
    parser.add_argument(
        ...
        default=20,
        type=int,
        help="number of epoch per decay [default: 20]",
    )
    parser.add_argument(
        "--n_workers",
        type=int,
        default=4,
        help="DataLoader Workers Number [default: 4]",
    )
    parser.add_argument(
        "--init", type=str, default=None, help="initializer for model [kaiming, xavier]"
    )

    # Evaluation
    parser.add_argument(
        "--min_val", type=int, default=100, help="Min val epoch [default: 100]"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sum",
        help="pooling for umbrella repsurf [sum, mean, max]",
    )
...
def main(args):
    ...
    if args.scheduler == "step":
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=args.decay_step, gamma=0.7
        )
    else:
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, data in enumerate(trainDataLoader):
            ...
            if batch_id % 80 == 0:
                print(
                    "Epoch: [{0}][{1}/{2}] lr {lr:.6f} loss {loss:.4f}".format(
                        epoch, batch_id, len(trainDataLoader), lr=lr, loss=loss.item()
                    )
                )
        ...
        log_string(
            "Train Instance Accuracy: %.2f, Loss: %f"
            % (train_instance_acc * 100, train_mean_loss)
        )

        if epoch >= args.min_val:
            with torch.no_grad():
                sing_acc, vote_acc = test(
                    classifier.eval(),
                    testDataLoader,
